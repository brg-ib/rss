...
output_rss_feed('https://www.lemonde.fr/ingenieurs-sciences/rss_full.xml', 20, true, true, 200);
...
function get_rss_feed_as_html($feed_url, $max_item_cnt = 10, $show_date = true, $show_description = true, $max_words = 0, $cache_timeout = 7200, $cache_prefix = "/tmp/rss2html-")
{
    ...
    for ($x=0;$x<$max_item_cnt;$x++) {
        ...
        if ($show_date) {
            ...
            $result .= '<small class="feed-date"><em>Posté le'.$date.'</em></small>';
        }
        if ($show_description) {$description = $feed[$x]['desc'];
            ...
            $result .= ' <a href="'.$link.'" title="'.$title.'">Lire la suite</a>'.'</div>';
        }
        $result .= '</li>';
    }
    $result .= '</ul>';
